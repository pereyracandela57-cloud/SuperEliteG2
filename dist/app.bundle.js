import{jsxDEV as X,Fragment as SJ}from"react/jsx-dev-runtime";const s1={apiKey:"AIzaSyAcrVq7ryWv5EJbRhZ1TmhFia-LAV10cN4",authDomain:"eliteg3-1650f.firebaseapp.com",databaseURL:"https://eliteg3-1650f-default-rtdb.firebaseio.com/",projectId:"eliteg3-1650f",storageBucket:"eliteg3-1650f.firebasestorage.app",messagingSenderId:"351711372153",appId:"1:351711372153:web:60ce4742396e37974f85a8",measurementId:"G-HGYPLEY6DJ"};if(!firebase.apps.length)firebase.initializeApp(s1);const w=firebase.database(),iY=firebase.storage(),{useState:V,useEffect:i,useMemo:f,useRef:zJ}=React,WJ=["C","P","B","N","S","E","X","R"],d1=["R"],jX="anonimo/galeria",GJ="__anonimo_gallery__",O0="eliteg3:perfiles-cache:v1",W0="eliteg3:gallery-audios-cache:v1",n1=["PERSONAJE","ETIQUETA","GENERAL"],L0={PERSONAJE:"Personaje",ETIQUETA:"Etiqueta",GENERAL:"General"},lJ=[{id:"perfil",label:"Perfil",labels:[]},{id:"colaPiernas",label:"Cola/Piernas",labels:["C"]},{id:"pechos",label:"Pechos",labels:["P"]},{id:"cuerpoCintura",label:"Cintura/Cuerpo",labels:["N","B"]},{id:"sensualidad",label:"Sensualidad",labels:["E","S"]}],p1={perfil:"perfil",pecho:"pechos",pechos:"pechos",cola:"colaPiernas",piernas:"colaPiernas",cuerpo:"cuerpoCintura",cintura:"cuerpoCintura",sensualidad:"sensualidad"},lY=()=>lJ.reduce((_,O)=>{_[O.id]="";return _},{}),EJ=(_={})=>{const O=lY(),D=_&&typeof _==="object"?_:{};return Object.keys(O).reduce((M,F)=>{M[F]=typeof D[F]==="string"?D[F].trim():"";return M},{...O})},_Y=(_="")=>lJ.find((O)=>O.id===_),i1=(_="")=>p1[(_||"").trim().toLowerCase()]||"",OY={C:{bg:"linear-gradient(135deg, rgba(34,211,238,0.92), rgba(14,116,144,0.95))",border:"rgba(103,232,249,0.95)",glow:"rgba(34,211,238,0.8)",text:"#ecfeff"},P:{bg:"linear-gradient(135deg, rgba(244,114,182,0.92), rgba(157,23,77,0.95))",border:"rgba(251,207,232,0.95)",glow:"rgba(244,114,182,0.78)",text:"#fff1f2"},B:{bg:"linear-gradient(135deg, rgba(251,191,36,0.94), rgba(180,83,9,0.95))",border:"rgba(253,224,71,0.98)",glow:"rgba(251,191,36,0.82)",text:"#fffbea"},N:{bg:"linear-gradient(135deg, rgba(163,230,53,0.92), rgba(63,98,18,0.95))",border:"rgba(217,249,157,0.95)",glow:"rgba(163,230,53,0.78)",text:"#f7fee7"},S:{bg:"linear-gradient(135deg, rgba(192,132,252,0.92), rgba(107,33,168,0.95))",border:"rgba(233,213,255,0.95)",glow:"rgba(192,132,252,0.8)",text:"#faf5ff"},E:{bg:"linear-gradient(135deg, rgba(248,113,113,0.92), rgba(153,27,27,0.95))",border:"rgba(254,202,202,0.95)",glow:"rgba(248,113,113,0.8)",text:"#fef2f2"},X:{bg:"linear-gradient(135deg, rgba(239,68,68,0.98), rgba(127,29,29,0.98))",border:"rgba(254,226,226,1)",glow:"rgba(239,68,68,0.95)",text:"#fff7f7"},R:{bg:"linear-gradient(135deg, rgba(148,163,184,0.95), rgba(71,85,105,0.98))",border:"rgba(226,232,240,0.95)",glow:"rgba(148,163,184,0.82)",text:"#f8fafc"},DEFAULT:{bg:"linear-gradient(135deg, rgba(51,65,85,0.92), rgba(15,23,42,0.95))",border:"rgba(148,163,184,0.85)",glow:"rgba(148,163,184,0.45)",text:"#f8fafc"}},qX=(_,O)=>{if(!O)return 0;return(_+O)%O},F0=(_,O,D=!1)=>{const M=Array.isArray(O)?O.length:0;if(!M)return null;if(M===1)return 0;if(D){const F=O.map((I,d)=>({item:I,idx:d})).filter(({item:I,idx:d})=>d!==_&&I&&I.url).map(({idx:I})=>I);if(!F.length)return qX((_??0)+1,M);return F[Math.floor(Math.random()*F.length)]}return qX((_??0)+1,M)},tJ=(_)=>OY[_]||OY.DEFAULT,WY=(_,O=!1)=>{const D=_==="INICIAL"?OY.DEFAULT:_==="100"?OY.B:tJ(_);return{"--btn-neon-color":D.glow,borderColor:D.border,color:D.text,boxShadow:O?`inset 0 1px 0 rgba(186,230,253,0.35), 0 0 0 1px ${D.border}, 0 0 20px ${D.glow}`:`inset 0 1px 0 rgba(186,230,253,0.2), 0 0 0 1px color-mix(in srgb, ${D.border} 65%, rgba(15,23,42,0.65) 35%), 0 0 12px color-mix(in srgb, ${D.glow} 60%, transparent)`,textShadow:`0 0 8px color-mix(in srgb, ${D.glow} 70%, transparent)`,filter:O?"brightness(1.14)":"brightness(1)"}},E0=/\.(mp4|webm|ogg|mov|m4v)(\?.*)?$/i,l1=/\.(mp3|wav|ogg|m4a|aac|flac)(\?.*)?$/i,t1=/\.gif(\?.*)?$/i,o1=/(?:youtube\.com\/(?:watch\?v=|embed\/|shorts\/)|youtu\.be\/)([A-Za-z0-9_-]{6,})/i,a1=/vimeo\.com\/(?:video\/)?(\d+)/i,_X=(_)=>{const O=(_||"").trim();if(!O)return null;const D=O.match(o1);if(D)return{provider:"youtube",src:`https://www.youtube.com/embed/${D[1]}`,title:"Video de YouTube"};const M=O.match(a1);if(M)return{provider:"vimeo",src:`https://player.vimeo.com/video/${M[1]}`,title:"Video de Vimeo"};return null},r1=["eporner.com"],U0=(_="")=>{const O=String(_||"").trim();if(!O||O.startsWith("data:")||O.startsWith("blob:"))return!1;try{const M=new URL(O).hostname.toLowerCase();return r1.some((F)=>M===F||M.endsWith(`.${F}`))}catch{return!1}},qJ=(_="",O="")=>{const D=String(_||"").trim();if(!D||U0(D))return O;return D},oJ=(_="",O="")=>{if(O==="video"||O==="image")return O;const D=(_||"").trim();if(E0.test(D)||_X(D))return"video";return"image"},V0=(_="")=>{return typeof _==="string"?_.trim():""},n=(_,O="")=>{if(typeof _==="string")return{url:qJ(_.trim(),""),label:"",type:oJ(_,O),autor:""};if(_&&typeof _==="object"){const D=qJ((_.url||"").trim(),"");return{url:D,label:WJ.includes(_.label)?_.label:"",type:oJ(D,_.type||O),autor:V0(_.autor)}}return{url:"",label:"",type:oJ("",O),autor:""}},e1=(_)=>n(_).url,D0=(_)=>n(_).label,aJ=(_)=>n(_).type,tY=(_,O,D="image")=>Array.isArray(_?.[O])?_[O].map((M)=>n(M,D)).filter((M)=>M.url):[],J2=(_={})=>({firebaseId:GJ,nombre:"Anónimo",nacionalidad:"",ciudad:"",profesion:"Galería",fechaNacimiento:"",estaturaCm:"",fotos:[],galeria:{fotos:tY(_,"fotos","image"),gifs:tY(_,"gifs","image"),videos:tY(_,"videos","video")},batallaFotosPreferidas:lY(),puntuaciones:xX(),isAnonymousGallery:!0}),C0=(_,O)=>{try{const D=window.localStorage.getItem(_);if(!D)return O;return JSON.parse(D)??O}catch{return O}},R0=(_,O)=>{try{window.localStorage.setItem(_,JSON.stringify(O))}catch{}},X2=(_={})=>{if(_?.isAnonymousGallery||_?.firebaseId===GJ)return"anonimo";return _?.firebaseId||""},Y2=(_={})=>{const O=qJ(String(_?.fotos?.[0]||"").trim(),""),D=EJ(_?.batallaFotosPreferidas||_?.galeria?.battlePhotoPreferences||{});if(!O)return!1;return lJ.filter((M)=>M.id!=="perfil").every((M)=>Boolean(String(D?.[M.id]||"").trim()))},Z2=(_={})=>{const O=[],D=qJ(String(_?.fotos?.[0]||"").trim(),""),M=EJ(_?.batallaFotosPreferidas||_?.galeria?.battlePhotoPreferences||{});if(!D)O.push("Perfil");lJ.filter((F)=>F.id!=="perfil").forEach((F)=>{if(!String(M?.[F.id]||"").trim())O.push(F.label)});return O},$2=(_=null)=>{if(!_)return;const O=window.open("","_blank");if(!O)return;const D=_?.firebaseId||_?.id||"",M=Array.isArray(_?.galeria?.fotos)?_.galeria.fotos.map((T)=>n(T,"image")).filter((T)=>T.url).map((T,G)=>({...T,sourceTag:"fotos",sourceIndex:G})):[],F=M.filter((T)=>T.type==="image"&&T.sourceTag==="fotos"),I=EJ(_?.batallaFotosPreferidas||_?.galeria?.battlePhotoPreferences||{}),d=qJ(String(_?.fotos?.[0]||_?.foto||"").trim(),""),$J=M.filter((T)=>T.type==="image"&&T.url).reduce((T,G)=>{T[G.url]=G;return T},{}),p=M.length?M.map((T,G)=>`
                    <button
                        type="button"
                ...
                `).join(""):'<p class="text-slate-300">Sin imágenes en galería.</p>',A=lJ.map((T)=>{const G=T.id==="perfil",P=G?d:I[T.id]||"",K=P?G?{url:P}:$J[P]||{url:P,label:"",sourceIndex:-1}:null,UJ=Boolean(P);return`
            ...
            <body>
                    ...
                    ${lJ.map((A)=>{const T=A.id==="perfil",G=T?p:$J[A.id]||"",P=Boolean(G),K=!T;return`
                            <div class="gallery-slot-card" data-slot-id="${A.id}" style="border:1px solid ${P?"rgba(74,222,128,0.95)":"rgba(248,113,113,0.95)"}; border-radius:10px; padding:10px; background:${P?"linear-gradient(155deg, rgba(20,83,45,0.95) 0%, rgba(21,128,61,0.82) 45%, rgba(5,46,22,0.96) 100%)":"linear-gradient(155deg, rgba(127,29,29,0.96) 0%, rgba(185,28,28,0.84) 45%, rgba(69,10,10,0.96) 100%)"}; box-shadow: inset 0 2px 0 rgba(255,255,255,0.24), inset 0 -2px 0 rgba(2,6,23,0.45), inset 0 0 0 1px ${P?"rgba(187,247,208,0.25)":"rgba(254,202,202,0.22)"}, 0 8px 16px rgba(2,6,23,0.45), 0 0 18px ${P?"rgba(74,222,128,0.35)":"rgba(248,113,113,0.3)"}; position: relative;">
                                ...
                                <div style="margin-top:8px; border-radius:8px; overflow:hidden; aspect-ratio:4/3; border:1px dashed rgba(148,163,184,0.45); background: rgba(2,6,23,0.86); display:flex; align-items:center; justify-content:center;">
                                    ${P?`<img src="${G}" alt="Vista previa ${A.label}" onerror="${oY}" style="width:100%; height:100%; object-fit:cover; display:block;" />`:'<span style="font-size:10px; color:#fca5a5; text-transform:uppercase; letter-spacing:0.08em; font-weight:700; text-align:center; padding:0 8px;">Sin foto designada</span>'}
                                </div>
                    ...
                    ${v.length?v.map((A,T)=>{const G=e1(A),P=D0(A),K=tJ(P),UJ={CANTANTE:{color:"#0ea5e9",sombra:"rgba(14,165,233,0.8)"},NSFW:{color:"#ef4444",sombra:"rgba(239,68,68,0.8)"},ACTRIZ:{color:"#22c55e",sombra:"rgba(34,197,94,0.8)"},MODELO:{color:"#f97316",sombra:"rgba(249,115,22,0.8)"},BAILARINA:{color:"#a855f7",sombra:"rgba(168,85,247,0.8)"},INFLUENCER:{color:"#ec4899",sombra:"rgba(236,72,153,0.8)"},PERIODISTA:{color:"#facc15",sombra:"rgba(250,204,21,0.8)"},ATLETA:{color:"#78350f",sombra:"rgba(120,53,15,0.8)"},OTRO:{color:"#ffffff",sombra:"rgba(255,255,255,0.8)"},DEFAULT:{color:"#334155",sombra:"transparent"}},c=UJ[D?.toUpperCase()]||UJ.DEFAULT;return`
                        ...
                        >
                           <button
                            ...
                            type="button"
                        >✕</button>
                        ${aJ(A)==="video"?(()=>{const MJ=_X(G);if(MJ)return`<div style="width: 100%; height: 100%; background: radial-gradient(circle at top, rgba(34,211,238,0.35), rgba(15,23,42,0.96)); display:flex; flex-direction:column; align-items:center; justify-content:center; gap:10px; padding: 18px; box-sizing:border-box; text-align:center;"><div style="width:64px; height:64px; border-radius:999px; border:1px solid rgba(34,211,238,0.5); background:rgba(15,23,42,0.75); display:flex; align-items:center; justify-content:center; color:#22d3ee; font-size:28px;">▶</div><div style="font-size:12px; font-weight:800; letter-spacing:0.12em; text-transform:uppercase; color:#e2e8f0;">Video embebido</div><div style="font-size:11px; color:#94a3b8; max-width:100%; word-break:break-word;">${MJ.provider.toUpperCase()}</div></div>`;return`<video src="${G}" style="width: 100%; height: 100%; object-fit: cover;" muted playsinline preload="metadata"></video>`})():`<img src="${G}" alt="Imagen de la galería" onerror="${oY}" style="width: 100%; height: 100%; object-fit: cover;" />`}
                        <div style="position:absolute; left: 12px; top: 12px; z-index: 15; padding: 6px 10px; border-radius: 999px; background: rgba(2,6,23,0.72); border: 1px solid rgba(148,163,184,0.24); color: #e2e8f0; display:flex; align-items:center; justify-content:center; font-size: 10px; font-weight: 900; letter-spacing: 0.2em; text-transform: uppercase; backdrop-filter: blur(8px);">${aJ(A)==="video"?"VIDEO":"IMG"}</div>
                ...
                <div id="fullscreenViewer" class="fullscreen-viewer">
                    ...
                    <div class="viewer-stage" id="viewerStage">
                        ${v.map((A,T)=>{const G=aJ(A),P=_X(A.url);return`
                            <div class="viewer-slide" id="viewer-slide-${T}">
                                ${G==="video"?P?`<iframe src="${P.src}" title="${P.title} ${T+1}" style="width:min(92vw, 1400px); height:min(75vh, 820px); border:0; border-radius:24px; box-shadow:0 0 40px rgba(34, 211, 238, 0.2); background:#000;" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>`:`<video src="${A.url}" controls playsinline preload="metadata" style="max-width:min(92vw, 1400px); max-height:calc(100vh - 64px); width:auto; height:auto; border-radius:24px; box-shadow:0 0 40px rgba(34, 211, 238, 0.2); background:#000;"></video>`:`<img src="${A.url}" alt="Vista completa ${T+1}" onerror="${oY}" />`}
                            </div>`}).join("")}
                    </div>
                </div>
