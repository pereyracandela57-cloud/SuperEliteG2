import{jsxDEV as X,Fragment as SJ}from"react/jsx-dev-runtime";const s1={apiKey:"AIzaSyAcrVq7ryWv5EJbRhZ1TmhFia-LAV10cN4",authDomain:"eliteg3-1650f.firebaseapp.com",databaseURL:"https://eliteg3-1650f-default-rtdb.firebaseio.com/",projectId:"eliteg3-1650f",storageBucket:"eliteg3-1650f.firebasestorage.app",messagingSenderId:"351711372153",appId:"1:351711372153:web:60ce4742396e37974f85a8",measurementId:"G-HGYPLEY6DJ"};if(!firebase.apps.length)firebase.initializeApp(s1);const K=firebase.database(),iY=firebase.storage(),{useState:D,useEffect:i,useMemo:f,useRef:zJ}=React,WJ=["C","P","B","N","S","E","X","R"],d1=["R"],jX="anonimo/galeria",GJ="__anonimo_gallery__",O0="eliteg3:perfiles-cache:v1",W0="eliteg3:gallery-audios-cache:v1",n1=["PERSONAJE","ETIQUETA","GENERAL"],L0={PERSONAJE:"Personaje",ETIQUETA:"Etiqueta",GENERAL:"General"},lJ=[{id:"perfil",label:"Perfil",labels:[]},{id:"colaPiernas",label:"Cola/Piernas",labels:["C"]},{id:"pechos",label:"Pechos",labels:["P"]},{id:"cuerpoCintura",label:"Cintura/Cuerpo",labels:["N","B"]},{id:"sensualidad",label:"Sensualidad",labels:["E","S"]}],p1={perfil:"perfil",pecho:"pechos",pechos:"pechos",cola:"colaPiernas",piernas:"colaPiernas",cuerpo:"cuerpoCintura",cintura:"cuerpoCintura",sensualidad:"sensualidad"},lY=()=>lJ.reduce((_,O)=>{_[O.id]="";return _},{}),EJ=(_={})=>{const O=lY(),C=_&&typeof _==="object"?_:{};return Object.keys(O).reduce((T,F)=>{T[F]=typeof C[F]==="string"?C[F].trim():"";return T},{...O})},_Y=(_="")=>lJ.find((O)=>O.id===_),i1=(_="")=>p1[(_||"").trim().toLowerCase()]||"",OY={C:{bg:"linear-gradient(135deg, rgba(34,211,238,0.92), rgba(14,116,144,0.95))",border:"rgba(103,232,249,0.95)",glow:"rgba(34,211,238,0.8)",text:"#ecfeff"},P:{bg:"linear-gradient(135deg, rgba(244,114,182,0.92), rgba(157,23,77,0.95))",border:"rgba(251,207,232,0.95)",glow:"rgba(244,114,182,0.78)",text:"#fff1f2"},B:{bg:"linear-gradient(135deg, rgba(251,191,36,0.94), rgba(180,83,9,0.95))",border:"rgba(253,224,71,0.98)",glow:"rgba(251,191,36,0.82)",text:"#fffbea"},N:{bg:"linear-gradient(135deg, rgba(163,230,53,0.92), rgba(63,98,18,0.95))",border:"rgba(217,249,157,0.95)",glow:"rgba(163,230,53,0.78)",text:"#f7fee7"},S:{bg:"linear-gradient(135deg, rgba(192,132,252,0.92), rgba(107,33,168,0.95))",border:"rgba(233,213,255,0.95)",glow:"rgba(192,132,252,0.8)",text:"#faf5ff"},E:{bg:"linear-gradient(135deg, rgba(248,113,113,0.92), rgba(153,27,27,0.95))",border:"rgba(254,202,202,0.95)",glow:"rgba(248,113,113,0.8)",text:"#fef2f2"},X:{bg:"linear-gradient(135deg, rgba(239,68,68,0.98), rgba(127,29,29,0.98))",border:"rgba(254,226,226,1)",glow:"rgba(239,68,68,0.95)",text:"#fff7f7"},R:{bg:"linear-gradient(135deg, rgba(148,163,184,0.95), rgba(71,85,105,0.98))",border:"rgba(226,232,240,0.95)",glow:"rgba(148,163,184,0.82)",text:"#f8fafc"},DEFAULT:{bg:"linear-gradient(135deg, rgba(51,65,85,0.92), rgba(15,23,42,0.95))",border:"rgba(148,163,184,0.85)",glow:"rgba(148,163,184,0.45)",text:"#f8fafc"}},qX=(_,O)=>{if(!O)return 0;return(_+O)%O},F0=(_,O,C=!1)=>{const T=Array.isArray(O)?O.length:0;if(!T)return null;if(T===1)return 0;if(C){const F=O.map((I,d)=>({item:I,idx:d})).filter(({item:I,idx:d})=>d!==_&&I&&I.url).map(({idx:I})=>I);if(!F.length)return qX((_??0)+1,T);return F[Math.floor(Math.random()*F.length)]}return qX((_??0)+1,T)},tJ=(_)=>OY[_]||OY.DEFAULT,WY=(_,O=!1)=>{const C=_==="INICIAL"?OY.DEFAULT:_==="100"?OY.B:tJ(_);return{"--btn-neon-color":C.glow,borderColor:C.border,color:C.text,boxShadow:O?`inset 0 1px 0 rgba(186,230,253,0.35), 0 0 0 1px ${C.border}, 0 0 20px ${C.glow}`:`inset 0 1px 0 rgba(186,230,253,0.2), 0 0 0 1px color-mix(in srgb, ${C.border} 65%, rgba(15,23,42,0.65) 35%), 0 0 12px color-mix(in srgb, ${C.glow} 60%, transparent)`,textShadow:`0 0 8px color-mix(in srgb, ${C.glow} 70%, transparent)`,filter:O?"brightness(1.14)":"brightness(1)"}},E0=/\.(mp4|webm|ogg|mov|m4v)(\?.*)?$/i,l1=/\.(mp3|wav|ogg|m4a|aac|flac)(\?.*)?$/i,t1=/\.gif(\?.*)?$/i,o1=/(?:youtube\.com\/(?:watch\?v=|embed\/|shorts\/)|youtu\.be\/)([A-Za-z0-9_-]{6,})/i,a1=/vimeo\.com\/(?:video\/)?(\d+)/i,_X=(_)=>{const O=(_||"").trim();if(!O)return null;const C=O.match(o1);if(C)return{provider:"youtube",src:`https://www.youtube.com/embed/${C[1]}`,title:"Video de YouTube"};const T=O.match(a1);if(T)return{provider:"vimeo",src:`https://player.vimeo.com/video/${T[1]}`,title:"Video de Vimeo"};return null},r1=["eporner.com"],U0=(_="")=>{const O=String(_||"").trim();if(!O||O.startsWith("data:")||O.startsWith("blob:"))return!1;try{const T=new URL(O).hostname.toLowerCase();return r1.some((F)=>T===F||T.endsWith(`.${F}`))}catch{return!1}},qJ=(_="",O="")=>{const C=String(_||"").trim();if(!C||U0(C))return O;return C},oJ=(_="",O="")=>{if(O==="video"||O==="image")return O;const C=(_||"").trim();if(E0.test(C)||_X(C))return"video";return"image"},V0=(_="")=>{return typeof _==="string"?_.trim():""},n=(_,O="")=>{if(typeof _==="string")return{url:qJ(_.trim(),""),label:"",type:oJ(_,O),autor:""};if(_&&typeof _==="object"){const C=qJ((_.url||"").trim(),"");return{url:C,label:WJ.includes(_.label)?_.label:"",type:oJ(C,_.type||O),autor:V0(_.autor)}}return{url:"",label:"",type:oJ("",O),autor:""}},e1=(_)=>n(_).url,D0=(_)=>n(_).label,aJ=(_)=>n(_).type,tY=(_,O,C="image")=>Array.isArray(_?.[O])?_[O].map((T)=>n(T,C)).filter((T)=>T.url):[],J2=(_={})=>({firebaseId:GJ,nombre:"Anónimo",nacionalidad:"",ciudad:"",profesion:"Galería",fechaNacimiento:"",estaturaCm:"",fotos:[],galeria:{fotos:tY(_,"fotos","image"),gifs:tY(_,"gifs","image"),videos:tY(_,"videos","video")},batallaFotosPreferidas:lY(),puntuaciones:xX(),isAnonymousGallery:!0}),C0=(_,O)=>{try{const C=window.localStorage.getItem(_);if(!C)return O;return JSON.parse(C)??O}catch{return O}},R0=(_,O)=>{try{window.localStorage.setItem(_,JSON.stringify(O))}catch{}},X2=(_={})=>{if(_?.isAnonymousGallery||_?.firebaseId===GJ)return"anonimo";return _?.firebaseId||""},Y2=(_={})=>{const O=qJ(String(_?.fotos?.[0]||"").trim(),""),C=EJ(_?.batallaFotosPreferidas||_?.galeria?.battlePhotoPreferences||{});if(!O)return!1;return lJ.filter((T)=>T.id!=="perfil").every((T)=>Boolean(String(C?.[T.id]||"").trim()))},Z2=(_={})=>{const O=[],C=qJ(String(_?.fotos?.[0]||"").trim(),""),T=EJ(_?.batallaFotosPreferidas||_?.galeria?.battlePhotoPreferences||{});if(!C)O.push("Perfil");lJ.filter((F)=>F.id!=="perfil").forEach((F)=>{if(!String(T?.[F.id]||"").trim())O.push(F.label)});return O},$2=(_=null)=>{if(!_)return;const O=window.open("","_blank");if(!O)return;const C=_?.firebaseId||_?.id||"",T=Array.isArray(_?.galeria?.fotos)?_.galeria.fotos.map((M)=>n(M,"image")).filter((M)=>M.url).map((M,w)=>({...M,sourceTag:"fotos",sourceIndex:w})):[],F=T.filter((M)=>M.type==="image"&&M.sourceTag==="fotos"),I=EJ(_?.batallaFotosPreferidas||_?.galeria?.battlePhotoPreferences||{}),d=qJ(String(_?.fotos?.[0]||_?.foto||"").trim(),""),$J=T.filter((M)=>M.type==="image"&&M.url).reduce((M,w)=>{M[w.url]=w;return M},{}),p=T.length?T.map((M,w)=>`
                    <button
                        type="button"
                ...
                `).join(""):'<p class="text-slate-300">Sin imágenes en galería.</p>',B=lJ.map((M)=>{const w=M.id==="perfil",P=w?d:I[M.id]||"",N=P?w?{url:P}:$J[P]||{url:P,label:"",sourceIndex:-1}:null,UJ=Boolean(P);return`
                ...
                <html lang="es">
                    ...
                    <body class="text-slate-200">
                        ...
                        <script>
                            ...
                            const slotConfigById = ${JSON.stringify(lJ.reduce((M,w)=>{M[w.id]=w;return M},{}))};
            ...
            <body>
                ...
                <details open style="width:100%; margin-bottom: 20px; border-radius: 12px; border: 1px solid rgba(148,163,184,0.28); background: rgba(2,6,23,0.45);">
                    ...
                    ${lJ.map((B)=>{const M=B.id==="perfil",w=M?p:$J[B.id]||"",P=Boolean(w),N=!M;return`
                            <div class="gallery-slot-card" data-slot-id="${B.id}" style="border:1px solid ${P?"rgba(74,222,128,0.95)":"rgba(248,113,113,0.95)"}; border-radius:10px; padding:10px; background:${P?"linear-gradient(155deg, rgba(20,83,45,0.95) 0%, rgba(21,128,61,0.82) 45%, rgba(5,46,22,0.96) 100%)":"linear-gradient(155deg, rgba(127,29,29,0.96) 0%, rgba(185,28,28,0.84) 45%, rgba(69,10,10,0.96) 100%)"}; box-shadow: inset 0 2px 0 rgba(255,255,255,0.24), inset 0 -2px 0 rgba(2,6,23,0.45), inset 0 0 0 1px ${P?"rgba(187,247,208,0.25)":"rgba(254,202,202,0.22)"}, 0 8px 16px rgba(2,6,23,0.45), 0 0 18px ${P?"rgba(74,222,128,0.35)":"rgba(248,113,113,0.3)"}; position: relative;">
                                ...
                                <div style="margin-top:8px; border-radius:8px; overflow:hidden; aspect-ratio:4/3; border:1px dashed rgba(148,163,184,0.45); background: rgba(2,6,23,0.86); display:flex; align-items:center; justify-content:center;">
                                    ${P?`<img src="${w}" alt="Vista previa ${B.label}" onerror="${oY}" style="width:100%; height:100%; object-fit:cover; display:block;" />`:'<span style="font-size:10px; color:#fca5a5; text-transform:uppercase; letter-spacing:0.08em; font-weight:700; text-align:center; padding:0 8px;">Sin foto designada</span>'}
                                </div>
                ...
                <div id="fullscreenViewer" class="fullscreen-viewer">
                    ...
                    <div class="viewer-stage" id="viewerStage">
                        ${v.map((B,M)=>{const w=aJ(B),P=_X(B.url);return`
